<template>
    <Card dis-hover>
        <table-header-new class="mb-14" title="用户提现"></table-header-new>
        ...
        <div class="display-flex mb-10">
            <Input class="flex-1" placeholder="请输入提现金额" v-model="amount">
                <span slot="prepend">提现金额</span>
            </Input>
            <div class="tool-amount mr-10">
                <Button :key="item.value" @click="setAmount(item.value)" v-for="item in tool">{{item.label}}</Button>
            </div>
            <Button @click="onConfirm" type="primary">确认提现</Button>
        </div>
    </Card>
</template>
...
export default {
    ...
    methods: {
        onConfirm() {
            ...
            if (!this.form.user_id) return this.$Message.warning('请输入提现用户ID')
            if (!this.form.nickname) return this.$Message.warning('请校验对方昵称')
            if (!this.form.amount) return this.$Message.warning('请输入提现金额')

            let message = '您确定要给<' + this.form.nickname + '>提现 ' + this.form.amount + ' 元吗？'
            ...
            this.loading = true
            this.$post('withdraw/create', this.form).then((res) => {
