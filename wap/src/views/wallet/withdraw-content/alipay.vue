    ...
    <div class="alipay">
        <div class="content">
            ...
            <div class="mt-1 bg-white content-intro">
                ...
                <span class="icon-intro color-pink">支付宝收款码</span>
            </div>
            ...
            <van-field class="mt-10" placeholder="请输入您的支付宝认证的姓名" v-model="form.username">
                <template #left-icon>
                    <van-icon color="#FF6A00" name="manager-o" size="16" />
                </template>
            </van-field>
            <van-field class="mt-10" placeholder="请输入您的支付宝账号" v-model="form.account">
                <template #left-icon>
...
export default {
    ...
    created() {
        if (this.withdraw.hasOwnProperty('alipay')) {
            this.form = this.withdraw['alipay'].extend
            this.qrcode = this.withdraw['alipay'].qrcode
        }
    },
    methods: {
        uploadQrcode(file) {
