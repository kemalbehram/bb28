    ...
    <div class="wechat">
        <div class="content">
            ...
            <div class="mt-1 bg-white content-intro">
                ...
                <span class="icon-intro color-pink">微信收款码</span>
            </div>
            ...
            <van-field class="mt-10" placeholder="请输入您的微信认证姓名" v-model="form.username">
                <template #left-icon>
                    <van-icon color="#FF6A00" name="manager-o" size="16" />
                </template>
            </van-field>
            <van-field class="mt-10" placeholder="请输入您的微信账号" v-model="form.account">
                <template #left-icon>
...
export default {
    ...
    created() {
        if (this.withdraw.hasOwnProperty('wechat')) {
            this.form = this.withdraw['wechat'].extend
            this.qrcode = this.withdraw['wechat'].qrcode
        }
    },
    methods: {
        uploadQrcode(file) {
