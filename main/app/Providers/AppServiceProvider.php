...
class AppServiceProvider extends ServiceProvider
{
    public function boot()
    {
        ...
        \OwenIt\Auditing\Models\Audit::creating(function (\OwenIt\Auditing\Models\Audit $model) {
            ...
            if ($model->event === 'created') {
                return false;
            }

            if ($model->auditable_type === 'App\Models\User') {
                if ($model->auditable_id > 1000000) {
                    return false;
                }
                if (isset($model->new_values['requested_at'])) {
                    return false;
                }
                if (isset($model->new_values['token'])) {
                    return false;
                }
                if (isset($model->new_values['requested_ip'])) {
                    return false;
                }
            }
        });
