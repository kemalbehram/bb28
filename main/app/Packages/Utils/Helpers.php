...
function toDecimal($value, $num = 1)
{
    return sprintf('%01.' . $num . 'f', $value);
}

function formatOdds($str, $length)
{
    $arr    = explode('.', $str);
 ...
 * 字符串转驼峰
 */
function toCamelCase($separator = '_', $value)
{
    if (strpos($value, $separator)) {
        $value = $separator . str_replace($separator, ' ', strtolower($value));
        $value = ltrim(str_replace(' ', '', ucwords($value)), $separator);
    }
...
function dateHandle($type = 'day', $start_date, $end_date)
{
    $params = [
        'day'   => 'Y-m-d',
        'month' => 'Y-m',
