...
class AuthController extends Controller
{
    public function get()
    {
        ...
        //获取在线客户TOKEN
        $user->service_token = auth('users')->tokenById(100000);
        // $user->ws_host       = env('WS_HOST') . ':' . env('WS_PORT');
        $user->ws_host = env('WS_HOST');
        // $user->access  = [env('ADMIN_ACCESS')];
        $result = $user->toArray();
    ...
    {
        // $agent_ip = request()->ip();
        // $allow_ip = config('admin.allow_ip');

        // if ($allow_ip !== '*' && in_array($agent_ip, $allow_ip) === false) {
        //     return real()->error('您暂无权限');
        // }
        $credentials = [
