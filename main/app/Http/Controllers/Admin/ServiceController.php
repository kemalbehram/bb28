...
class ServiceController extends Controller
{
    ...
    {
        $count  = PushMessage::where('read', 0)->where('to', 100000)->count();
        $result = ['unread' => $count];
    ...
    public function last()
    {
        ...
        foreach ($group as $value) {
            ...
            foreach ($value as $_value) {
                if ($_value['read'] == false && $_value['to'] == 100000) {
                    $unread = true;
                    $new    = true;
                    continue;
                }
    ...
    public function read(Request $request)
    {
        ...
        $temp = PushMessage::where('from', $request->id)->where('to', 100000)->where('read', 0)->update($update);
    ...
    public function send(Request $request)
    {
        ...
        PushEvent::name('service')->toUser(100000)->data($result);
