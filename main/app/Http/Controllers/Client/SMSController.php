...
class SMSController extends Controller
{
    ...
    private function bindAccount(Request $request)
    {
        $this->user && $this->user->CheckIsTrial();
        $result = $this->sendMessage('bind', $request->mobile);
    ...
    private function security()
    {
        ...
        $result = $this->sendMessage('security', $this->user->mobile);
        return real($result)->success('ver_code.get.success');
    }

    private function sendMessage($prefix, $mobile)
    {
        return SMS::send($prefix, $mobile, $code = null);
    }
}
